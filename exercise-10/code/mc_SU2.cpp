...
mc_su2::mc_su2 (int T, int L, double beta, unsigned int seed) :
		grid (T, L, L, L),
		U (grid),
		beta_(beta), rng(seed),
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 8) {
        ...
        std::cout << "T, L, beta, n_thermalisation, n_measurements, heatbath sweeps / MC update, over-relaxation updates / MC update" << std::endl;
        std::cout << "e.g. ./MC_SU2 4 8 0.1 1e2 1e3 1 5" << std::endl;
        return 1;
    }
	...
	int n_thermalisation = static_cast<int>(atof(argv[4]));
	int n_measurements = static_cast<int>(atof(argv[5]));
	int n_heatbath = static_cast<int>(atof(argv[6]));
	int n_overrelaxation = static_cast<int>(atof(argv[7]));
	...
	mc_su2 MC_SU2 (T, L, beta, seed);
	MC_SU2.init_random_field();
	//MC_SU2.init_unit_field();

	// thermalisation
	//std::cout << "# plaquette" << std::endl;
	for(int i=0; i<n_thermalisation; ++i){
		//std::cout << "# " << MC_SU2.plaq() << std::endl;
		MC_SU2.hb_sweep(n_heatbath);
		MC_SU2.or_sweep(n_overrelaxation);
	}

	// measurements: output measurements for later analysis
	std::cout << "# polyakov_loop, 1x1 wilson loop" << std::endl;
	for(int i=0; i<n_measurements; ++i){
		MC_SU2.hb_sweep(n_heatbath);
		MC_SU2.or_sweep(n_overrelaxation);
		std::cout << MC_SU2.polyakov_loop() << "\t" 
				  << MC_SU2.wilson_loop(1) << std::endl;
	}
